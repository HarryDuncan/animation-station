// File Of Generic Scenes - Used as a base
import * as THREE from "three";
import {fragmentShading} from './shaders/my-frag.js'
import {vertexShading} from './shaders/my-vert.js'
import {fractionate, avg, max, modulate} from './visualizerFunctions/visualizerMaths.js'
...
export function cubeAndSpline (framework){
    let scene = new THREE.Scene();
    let camera = new THREE.PerspectiveCamera(85, window.innerWidth / window.innerHeight, 0.1, 1000);
    camera.position.set(0, document.documentElement.clientWidth <= 900 ? -20 : 0, document.documentElement.clientWidth <= 900 ? 120 : 100);
    camera.lookAt(new THREE.Vector3(0,0,0));
    ...
    // let icosahedronGeometry = new THREE.BoxGeometry(50, 70, 50);
    // let material = new THREE.TextureLoader().load('../images/textures/2.jpg')
   ...
   //  group.add(cube);
    ...
    let candS = {
        ...
        onUpdate: function(framework) {
          ...
          function animateCamera(framework, scale, pipeSpline) {
            var splineCamera = framework.camera;
            var tubeGeometry = new THREE.TubeBufferGeometry(pipeSpline, 100, 8, 3, true);
            var binormal = new THREE.Vector3();
            var normal = new THREE.Vector3();
