...
const NavMenu: React.FunctionComponent<INavMenuProps> = (props) => {
  const _generateNavMenuItems = (
    linkClickCallback: (arg: string) => void,
    playlistItems?: IPlayListItem[]
  ) => {
    ...
    const _renderPlaylistLinks = (playlistItems: IPlayListItem[]) => {
      ...
      let playlistLinks = playlistItems.map((item, index) => ({
        name: item.playlistTitle,
        onClick: () => linkClickCallback(item.playlistId),
        key: `${item.playlistId} ${item.playlistTitle}`,
      }));
  ...
  return (
    <Nav
      ...
      styles={navStyles}
      groups={_generateNavMenuItems(
        props.itemClickedCallback,
        props.playlistItems
      )}
    />
  );
