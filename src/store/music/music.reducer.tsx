...
function music(state: IMusicState = initialMusicState, action: any) {
  switch (action.type) {
    ...
    case musicActionTypes.NEW_PLAYLIST:
      return update(state, {
        ...
        playlists: {
          $push: [
            {
              playlistId: state.playlists.length,
              playlistTitle: `New Playlist ${state.playlists.length + 1}`,
              playlistTracks: [],
              saved: false,
            },
          ],
    ...
    case musicActionTypes.ADD_TO_PLAYLIST:
      ...
      newPlaylists[state.playlistManagerIndex]["playlistTracks"] = newPlaylists[
        state.playlistManagerIndex
      ]["playlistTracks"].concat(
        action.payload.map((item, index) => ({
          trackTitle: item["name"],
          trackId:
            state.playlists[state.playlistManagerIndex]["playlistTracks"]
              .length + index,
          src: item["path"],
          trackType: item["type"],
        }))
